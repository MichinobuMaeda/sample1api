...
UI として Vue で作成したアプリ https://github.com/MichinobuMaeda/sample3spa が利用できる。
...
このページを直接参照せずにコピーしたものを利用すると古いバージョンを使うことになってしまうので、必ずこのページに記載された最新のコマンドを実行すること。
Linux のディストリビューションのパッケージマネージャ等でインストールできる場合はそちらでもよいが、たぶんこのページが本当の正式の最新版。
